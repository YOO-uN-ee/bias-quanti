...
def limit_vocab(wv, w2i, vocab, exclude=None):
    WEAT = [word.lower() for k,v in WEAT_words.items() for word in v ]
    flowers = 'aster, clover, hyacinth, marigold, poppy, azalea, crocus, iris, orchid, rose, bluebell, daffodil, lilac, pansy, tulip, buttercup, daisy, lily, peony, violet, carnation, magnolia, petunia, zinnia'.split(
        ', ')[:10]
    insects = 'ant, caterpillar, flea, locust, spider, bedbug, centipede, fly, maggot, tarantula, bee, cockroach, gnat, mosquito, termite, beetle, cricket, hornet, moth, wasp, blackfly, dragonfly, horsefly, roach'.split(
        ', ')[:10]
    positive = 'caress, freedom, health, love, peace, cheer, friend, heaven, loyal, pleasure, diamond, gentle, honest, lucky, rainbow, diploma, gift, honor, miracle, sunrise, family, happy, laughter, paradise, vacation'.split(
        ', ')[:10]
    ...
    WEAT.extend(flowers)
    WEAT.extend(insects)
    WEAT.extend(positive)
    WEAT.extend(negative)
    WEAT.extend(eu_american_names)
    WEAT.extend(af_american_names)
    WEAT.extend(old_name)
    WEAT.extend(young_name)
